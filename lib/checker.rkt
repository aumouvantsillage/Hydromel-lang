...
(define-syntax-parser compile-as-module-level-defs
  ...
  [(_ s:stx/interface)
   #'(begin
       (provide s.name)
       (define-syntax s.name (meta/make-interface)))]

  [(_ s:stx/component)
   #'(begin
       (provide s.name)
       (define-syntax s.name (meta/make-component)))]
...
(begin-for-syntax
  ...
  (define (make-checker stx)
    (syntax-parse stx
      ...
      [s:stx/interface
       (define body^
         (parameterize ([current-design-unit-name #'s.name])
           (with-scope
             (~>> (attribute s.body)
                  (create-aliases)
                  (map add-scope)
      ...
      [s:stx/parameter
       #:with name^ (bind! #'s.name (meta/parameter #'s.name))
       ; Add element to the public fields of the parent design unit.
       (~> (current-design-unit-name)
           (lookup)
           (meta/design-unit-set-from-scope! #'name^))
       (thunk/in-scope
         ; TODO check type
         #'(parameter name^ s.type))]

      [s:stx/data-port
       #:with name^ (bind! #'s.name (meta/data-port #'s.name (syntax-e #'s.mode)))
       ; Add element to the public fields of the parent design unit.
       (~> (current-design-unit-name)
           (lookup)
           (meta/design-unit-set-from-scope! #'name^))
      ...
      [s:stx/composite-port
       #:with name^ (bind! #'s.name (meta/composite-port #'s.name #'s.intf-name (attribute s.flip?)))
      ...
      [s:stx/assignment
       ...
       (define expr^   (make-checker #'s.expr))
       (thunk/in-scope
         #`(assignment #,(check-assignment-target (target^)) #,(check-assigned-expr (expr^))))]
  ...
  (define (flip m)
    (match m
      [(meta/data-port      name mode)            (meta/data-port      name (if (eq? 'in mode) 'out 'in))]
      [(meta/composite-port name intf-name flip?) (meta/composite-port name intf-name (not flip?))]
      [_                                     m]))
      ...
      (for/list ([stx (in-list lst)])
        (syntax-parse stx
          [s:stx/composite-port #:when (attribute s.splice?)
           (define intf (lookup #'s.intf-name meta/interface?))
           (displayln (meta/design-unit-public-fields intf))
           (cons stx
             (for/list ([field (in-dict-values (meta/design-unit-public-fields intf))])
               (displayln (meta/field-name field))
               (define field-name^ (bind! (meta/field-name field)
                                          (if (attribute s.flip?) (flip field) field)))
               #`(alias #,field-name^ s.name s.intf-name)))]
    ...
    (syntax-parse stx
      ...
      [s:stx/field-expr
       ...
       (match (resolve #'s.expr)
         [(meta/composite-port _ intf-name _)
  ...
  (define (check-assignment-target stx)
    ...
    (syntax-parse stx
      ...
      [s:stx/field-expr
       ; For a field expression, resolve the left-hand side first.
       (match (resolve #'s.expr)
         [(meta/composite-port _ intf-name flip?)
          ; If the lhs maps to a composite port, look up the given field name
          ; in the target interface.
