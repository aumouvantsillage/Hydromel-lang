...
(define-syntax-parser compile-as-module-level-defs
  ...
  [(_ s:stx/interface)
   #'(begin
       (provide s.name)
       (define-syntax s.name (meta/interface empty)))]

  [(_ s:stx/component)
   #'(begin
       (provide s.name)
       (define-syntax s.name (meta/component empty)))]

  [_
   #'(begin)])

; Second pass: fill the metadata of module-level elements (see fill-module-level-metadata).
; Third pass:  create the scope hierarchy and bindings to local metadata (see make-checker).
; Fourth pass: perform semantic checking (see thunks generated by make-checker).
(define-syntax (compile-hydromel stx)
  (define/syntax-parse (_ body ...) stx)
  (fill-module-level-metadata (attribute body))
  ((make-checker stx)))

(begin-for-syntax
  ; Second pass: fill the metadata of module-level elements.
  (define (fill-module-level-metadata lst)
    (for ([it (in-list lst)])
      (syntax-parse it
        [s:stx/design-unit
         (~> #'s.name
             (lookup)
             (meta/set-design-unit-ports!
               (filter identity
                 (for/list ([stx (in-list (attribute s.body))])
                   (syntax-parse stx
                     [p:stx/data-port
                      (cons (syntax-e #'p.name) (meta/data-port #'p.name (syntax-e #'p.mode)))]
                     [p:stx/composite-port
                      (cons (syntax-e #'p.name) (meta/composite-port #'p.name #'p.intf-name (attribute p.flip?) (attribute p.splice?)))]
                     [_ #f])))))]
        [_ (void)])))
  ...
  ; Third pass:  create the scope hierarchy and bindings to local metadata.
  ; Fourth pass: perform semantic checking in returned thunks.
  ...
  (define (check-assignment-target stx)
    ...
    (syntax-parse stx
      ...
      [s:stx/field-expr
       ; For a field expression, resolve the left-hand side first.
       (match (resolve #'s.expr)
         [(meta/composite-port _ intf-name flip? _)
          ; If the lhs maps to a composite port, look up the given field name
          ; in the target interface.
