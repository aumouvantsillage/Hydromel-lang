...
(let ([s (make-signal 42)])
  (check-equal? (signal-take s 3) (list 42 42 42)))

(let ([s (make-signal 10 20 30)])
  (check-equal? (signal-take s 5) (list 10 20 30 30 30)))
...
(let* ([signal+ (signal-lift +)]
       [s1 (make-signal   1   2   3)]
       [s2 (make-signal  10  20  30)]
       [s3 (make-signal 100 200 300)]
       [s4 (signal+ s1 s2 s3)])
  (check-equal? (signal-take s4 3) (list 111 222 333)))

(let* ([signal-if (signal-lift* if _ _ _)]
       [s1 (make-signal  #f  #t  #f  #t)]
       [s2 (make-signal  10  20  30  40)]
       [s3 (make-signal 100 200 300 400)]
...
(let ()
  (define-signal (signal+ . lst)
    (apply + lst))
  (define s1 (make-signal   1   2   3))
  (define s2 (make-signal  10  20  30))
  (define s3 (make-signal 100 200 300))
...
(let ()
  (define-signal (signal-if c t e)
    (if c t e))
  (define s1 (make-signal  #f  #t  #f  #t))
  (define s2 (make-signal  10  20  30  40))
  (define s3 (make-signal 100 200 300 400))
  (define s4 (signal-if s1 s2 s3))
  (check-equal? (signal-take s4 4) (list 100 20 300 40)))

(let* ([s1 (make-signal  #f  #t  #f  #t)]
       [s2 (make-signal  10  20  30  40)]
       [s3 (make-signal 100 200 300 400)]
...
(let ([s (register 10 (make-signal 20 30 40))])
...
(let* ([s1 (make-signal #f #f #t #f)]
       [s2 (register/r 10 s1 (make-signal 20 30 40 50 60))])
  (check-equal? (signal-take s2 6) (list 10 20 30 10 50 60)))

(let* ([s1 (make-signal #f #t #f #t #f)]
       [s2 (register/e 10 s1 (make-signal 20 30 40 50 60))])
  (check-equal? (signal-take s2 6) (list 10 10 30 30 50 50)))

(let* ([s1 (make-signal #f #f #t #f #f #t #f)]
       [s2 (make-signal #f #t #f #t #f #t #f)]
       [s3 (register/re 10 s1 s2 (make-signal 20 30 40 50 60 70))])
  (check-equal? (signal-take s3 7) (list 10 10 30 10 50 50 10)))

(let* ([s1 (make-signal   1   2   3)]
       [s2 (make-signal  10  20  30)]
       [s3 (make-signal 100 200 300)]
       [s4 (signal-bundle s1 s2 s3)])
  (check-equal? (signal-take s4 3) (list (list 1 10 100) (list 2 20 200) (list 3 30 300))))

(let*-values ([(s1)       (make-signal (list 1 10 100) (list 2 20 200) (list 3 30 300))]
...
(let*-values ([(s1) (make-signal   1   2   3)]
              [(s2) (make-signal  10  20  30)]
              [(s3) (make-signal 100 200 300)]
...
(let ()
  (define signal-inc3 (signal-λ (a #:b b [c (make-signal 100 200 300)] #:d [d (make-signal 1000 2000 3000)])
                        #:returns (w x y z)
                        (define w (add1 a))
                        (define x (add1 b))
                        (define y (add1 c))
...
(let ()
  (define-signal (signal-inc3 a #:b b [c (make-signal 100 200 300)] #:d [d (make-signal 1000 2000 3000)])
    #:returns (w x y z)
    (define w (add1 a))
    (define x (add1 b))
    (define y (add1 c))
